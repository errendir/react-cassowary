...
type ContainerProps = {
  debug?: boolean,
  children: (a: { [key: string]: DimensionVariables }) => React.ReactNode
}
type ContainerState = {
  dirty: boolean
  layout
...
export class Container extends React.Component<ContainerProps, ContainerState> {
  ...
  private variables = {}
  private variableGeneratorProxy: { [key: string]: DimensionVariables } = new Proxy({}, {
    get: (_obj, variableName: string): DimensionVariables => {
      if (!this.variables[variableName]) {
        console.log("generating dimension variables", variableName)
        ...
        // console.log("reusing dimension variables", variableName)
      }
    ...
    return <>
      <ConstraintSetProvider value={this.constraintSet}>
        <LayoutProvider value={this.state.layout}>
          {VariableConsumer(this.variableGeneratorProxy)}
        </LayoutProvider>
...
class ItemPositioned extends React.Component<{ layout: any, } & ItemProps, { width, height }> {
  ...
  render() {
    ...
    return <div style={this.computeStyleFromLayout()}>
